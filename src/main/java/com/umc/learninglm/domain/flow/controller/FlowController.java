...
import com.umc.learninglm.domain.flow.service.FlowPreviewService;
import com.umc.learninglm.domain.flow.service.FlowService;
...
public class FlowController {
	...
	private final FlowPreviewService flowPreviewService;
	...
	@ApiResponses({
			...
			@ApiResponse(responseCode = "404", description = "TUTORIAL40401: 존재하지 않는 tutorialId / FLOW40403: 존재하지 않는 originFlowId"),
	...
	public BaseResponse<FlowCreateResponse> createFlow(@Valid @RequestBody FlowCreateRequest request) {
		return BaseResponse.success(flowService.createFlow(request));
	}
	...
	public BaseResponse<FlowDetailResponse> getFlow(@PathVariable Long flowId) {
		return BaseResponse.success(flowService.getFlow(flowId));
	}
	...
	@ApiResponses({
			...
			@ApiResponse(responseCode = "400", description = "FLOW40001: 잘못된 요청 / FLOW40003: options 파싱 실패"),
			@ApiResponse(responseCode = "403", description = "FLOW40301: 소유자 불일치"),
	...
	public BaseResponse<FlowUpdateResponse> updateFlow(
		...
		return BaseResponse.success(flowService.updateFlow(flowId, request));
	}

	@DeleteMapping("/{flowId}")
	@Operation(summary = "흐름 삭제", description = "사용자 소유 흐름을 삭제합니다. 삭제 시 flow_blocks, flow_categories, flow_likes, flow_comments, flow_bookmarks, flow_tags가 함께 제거됩니다.",
			security = @SecurityRequirement(name = "bearerAuth"))
	@ApiResponses({
	...
	public BaseResponse<FlowDeleteResponse> deleteFlow(@PathVariable Long flowId) {
		return BaseResponse.success(flowService.deleteFlow(flowId));
	}
	...
	public BaseResponse<FlowVerifyResponse> verifyFlow(
		...
		return BaseResponse.success(flowService.verifyFlow(flowId, request));
	}
	...
	@ApiResponses({
			...
			@ApiResponse(responseCode = "403", description = "FLOW40301: 소유자 불일치"),
			@ApiResponse(responseCode = "404", description = "FLOW40401: 존재하지 않는 흐름 / FLOW40402: 잘못된 blockId"),
			@ApiResponse(responseCode = "401", description = "AUTH40103~AUTH40104: Access Token 오류")
	})
	public BaseResponse<FlowPreviewResponse> previewFlow(
			@PathVariable Long flowId, @Valid @RequestBody FlowPreviewRequest request) {
		return BaseResponse.success(flowPreviewService.previewFlow(flowId, request));
	}
}
